...
class UserApp {
    ...
    constructor(options = {}){  
        ...
        this.db = new UserDatabase(options);
        return this;
    }
    ...
    async loginUser(username, password, ip, browser){
        ...
        if(!user.sessionId){
            this.logger.debug("Failed to login user: cannot generate session");
            return null;
        }

        let result = await this.db.addSession(user._id, user.sessionId, ip, browser);
        if(!result){
            this.logger.debug("Failed to login user: cannot add session");
            return null;
        }
