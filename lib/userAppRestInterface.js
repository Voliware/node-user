...
class UserAppRestInterface {
    ...
    async getClientUser(request, response){
        ...
        return this.app.loginUserWithSessionId(sessionId, client.ip, client.browser);
    }
    ...
    async getUser(request, response, data){
        ...
        if(user){
            return this.http_server.sendJson(response, user);
        }
        else{
    ...
    async getUsers(request, response, data){
        ...
        if(users){
            return this.http_server.sendJson(response, users);
        }
        else{
    ...
    async loginUser(request, response, data){
        let clientUser = await this.getClientUser(request, response);
        if(clientUser){
            return this.http_server.sendJson(response, clientUser);
        }

        if(!data.body){
            // 401 here because it is an auth attempt
            return this.http_server.sendStatusCode(response, 401);
        }
    ...
    attachHttpServerHandlers(http_server){
        http_server.addRoute("GET", "/user/:id", (request, response, data) => {
            this.getUser(request, response, data);
        });
        http_server.addRoute("DELETE", "/user/:id", (request, response, data) => {
            this.deleteUser(request, response, data);
        });
        http_server.addRoute("GET", "/users", (request, response, data) => {
            this.getUsers(request, response, data);
        });
        http_server.addRoute("POST", "/user/add", (request, response, data) => {
            this.addUser(request, response, data);
        });
        http_server.addRoute("PUT", "/user/update", (request, response, data) => {
            this.updateUser(request, response, data);
        });
        http_server.addRoute("POST", "/user/register", (request, response, data) => {
            this.registerUser(request, response, data);
        });
        http_server.addRoute("POST", "/user/reset", (request, response, data) => {
            this.resetPassword(request, response, data);
        });
        http_server.addRoute("POST", "/user/verify", (request, response, data) => {
            this.verifyUser(request, response, data);
        });
        http_server.addRoute("POST", "/user/login", async (request, response, data) => {
            this.loginUser(request, response, data);
        });
        http_server.addRoute("POST", "/user/logout", (request, response, data) => {
